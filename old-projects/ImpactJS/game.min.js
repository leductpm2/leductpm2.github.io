...
else if(this.health<=5*multiply){this.currentAnim=this.anims.heal5;}},update:function(){this.parent();this.onhealthChange();},kill:function(){this.parent();ig.game.base_canon.kill();this.sfxDie.play();setTimeout(function(){ig.game.loadLevelDeferred(LevelGame_over_screen);},1000);},});});

// lib/game/entities/base_canon.js
ig.baked=true;ig.module('game.entities.base_canon').requires('impact.entity').defines(function(){EntityBase_canon=ig.Entity.extend({size:{x:72,y:50},flip:false,nextShootTime:0,animSheet:new ig.AnimationSheet('media/small/island_canon.png',72,50),sfxFire:new ig.Sound('media/sounds/FireCanon.mp3'),init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('idle',1,[0]);this.addAnim('shoot',0.1,[0,1,2,3,4]);this.currentAnim=this.anims.idle;ig.game.base_canon=this;this.shootPos={x:this.pos.x+this.size.x/2,y:this.pos.y+this.size.y/2}},shoot:function(target){if(this.nextShootTime<=Date.now()){this.currentAnim=this.anims.shoot.rewind();ig.game.spawnEntity(EntityCanonball,this.pos.x+this.size.x/2,this.pos.y+this.size.y/2,{canonPos:this.shootPos,targetPos:target,flip:this.flip});this.nextShootTime=Date.now()+(base_canon_rate_of_fire/ig.Timer.timeScale);this.sfxFire.play();}},update:function(){if(ig.input.pressed('shoot')&&!ig.game.pointer.isOnUI){this.flip=ig.input.mouse.x<this.shootPos.x;this.shoot(ig.input.mouse);}
if(this.currentAnim==this.anims.shoot&&this.currentAnim.loopCount>0){this.currentAnim=this.anims.idle;}
...
ig.baked=true;ig.module('game.entities.enemy_ship_2').requires('impact.entity').defines(function(){EntityEnemy_ship_2=ig.Entity.extend({_wmScalable:true,_wmDrawBox:true,_wmBoxColor:'rgba(255, 0, 0, 0.7)',type:ig.Entity.TYPE.B,size:{x:132,y:56},offset:{x:0,y:40},gravityFactor:0,ready_to_shoot:false,nextShootTime:0,spawnPos:null,animSheet:new ig.AnimationSheet('media/small/ship_2_sprite.png',132,96),sfxFire:new ig.Sound('media/sounds/FireEnemy.mp3'),init:function(x,y,settings){this.parent(x,y,settings);this.flip=settings.flip;this.health=settings.maxHealth;this.spawnPos=settings.spawnPos;this.zIndex=settings.zIndex;this.addAnim('idle',1,[0]);this.addAnim('shoot',0.2,[0,1,2,3,4,5]);this.currentAnim=this.anims.idle;this.vel.x=(this.flip?1:-1)*enemy_speed;this.target_posX=this.pos.x+(this.flip?settings.distance-this.size.x:-settings.distance);},shoot:function(target){this.currentAnim=this.anims.shoot.rewind();ig.game.spawnEntity(EntityBullet_enemy,this.flip?this.pos.x+this.size.x:this.pos.x,this.pos.y,{target:target,flip:this.flip});this.nextShootTime=Date.now()+(enemy_rate_of_fire/ig.Timer.timeScale);this.sfxFire.play();},update:function(){if(this.currentAnim==this.anims.shoot&&this.currentAnim.loopCount>0){this.currentAnim=this.anims.idle;}
if(!this.ready_to_shoot){if(this.target_posX<this.pos.x&&this.flip||this.target_posX>this.pos.x&&!this.flip||this.target_posX==this.pos.x){this.ready_to_shoot=true;this.vel.x=0;this.nextShootTime=Date.now()+(enemy_rate_of_fire/ig.Timer.timeScale);}}
else
{if(this.nextShootTime<=Date.now())
{this.shoot(ig.game.base.pos);}}
this.currentAnim.flip.x=this.flip;this.currentAnim.update();this.parent();},kill:function(){this.parent();current_score+=1;this.spawnPos.spawned=false;this.spawnPos.spawEnemy();},});});

// lib/game/entities/enemy_ship_1.js
ig.baked=true;ig.module('game.entities.enemy_ship_1').requires('impact.entity').defines(function(){EntityEnemy_ship_1=ig.Entity.extend({_wmScalable:true,_wmDrawBox:true,_wmBoxColor:'rgba(255, 0, 0, 0.7)',type:ig.Entity.TYPE.B,size:{x:128,y:54},offset:{x:0,y:20},gravityFactor:0,ready_to_shoot:false,nextShootTime:0,spawnPos:null,animSheet:new ig.AnimationSheet('media/small/ship_1_sprite.png',128,74),sfxFire:new ig.Sound('media/sounds/FireEnemy.mp3'),init:function(x,y,settings){this.parent(x,y,settings);this.flip=settings.flip;this.health=settings.maxHealth;this.spawnPos=settings.spawnPos;this.zIndex=settings.zIndex;this.addAnim('idle',1,[0]);this.addAnim('shoot',0.2,[0,1,2,3,4,5]);this.currentAnim=this.anims.idle;this.vel.x=(this.flip?1:-1)*enemy_speed;this.target_posX=this.pos.x+(this.flip?settings.distance-this.size.x:-settings.distance);},shoot:function(target){this.currentAnim=this.anims.shoot.rewind();ig.game.spawnEntity(EntityBullet_enemy,this.flip?this.pos.x+this.size.x:this.pos.x,this.pos.y,{target:target,flip:this.flip});this.nextShootTime=Date.now()+(enemy_rate_of_fire/ig.Timer.timeScale);this.sfxFire.play();},update:function(){if(this.currentAnim==this.anims.shoot&&this.currentAnim.loopCount>0){this.currentAnim=this.anims.idle;}
if(!this.ready_to_shoot){if(this.target_posX<this.pos.x&&this.flip||this.target_posX>this.pos.x&&!this.flip||this.target_posX==this.pos.x){this.ready_to_shoot=true;this.vel.x=0;this.nextShootTime=Date.now()+(enemy_rate_of_fire/ig.Timer.timeScale);}}
else{if(this.nextShootTime<=Date.now()){this.shoot(ig.game.base.pos);}}
this.currentAnim.flip.x=this.flip;this.currentAnim.update();this.parent();},kill:function(){this.parent();current_score+=1;this.spawnPos.spawned=false;this.spawnPos.spawEnemy();},});});

// lib/game/entities/enemy_ship_3.js
ig.baked=true;ig.module('game.entities.enemy_ship_3').requires('impact.entity').defines(function(){EntityEnemy_ship_3=ig.Entity.extend({_wmScalable:true,_wmDrawBox:true,_wmBoxColor:'rgba(255, 0, 0, 0.7)',type:ig.Entity.TYPE.B,size:{x:118,y:52},offset:{x:0,y:20},gravityFactor:0,ready_to_shoot:false,nextShootTime:0,spawnPos:null,animSheet:new ig.AnimationSheet('media/small/ship_3_sprite.png',118,72),sfxFire:new ig.Sound('media/sounds/FireEnemy.mp3'),init:function(x,y,settings){this.parent(x,y,settings);this.flip=settings.flip;this.health=settings.maxHealth;this.spawnPos=settings.spawnPos;this.zIndex=settings.zIndex;this.addAnim('idle',1,[0]);this.addAnim('shoot',0.2,[0,1,2,3,4,5]);this.currentAnim=this.anims.idle;this.vel.x=(this.flip?1:-1)*enemy_speed;this.target_posX=this.pos.x+(this.flip?settings.distance-this.size.x:-settings.distance);},shoot:function(target){this.currentAnim=this.anims.shoot.rewind();ig.game.spawnEntity(EntityBullet_enemy,this.flip?this.pos.x+this.size.x:this.pos.x,this.pos.y,{target:target,flip:this.flip});this.nextShootTime=Date.now()+(enemy_rate_of_fire/ig.Timer.timeScale);this.sfxFire.play();},update:function(){if(this.currentAnim==this.anims.shoot&&this.currentAnim.loopCount>0){this.currentAnim=this.anims.idle;}
if(!this.ready_to_shoot){if(this.target_posX<this.pos.x&&this.flip||this.target_posX>this.pos.x&&!this.flip||this.target_posX==this.pos.x){this.ready_to_shoot=true;this.vel.x=0;this.nextShootTime=Date.now()+(enemy_rate_of_fire/ig.Timer.timeScale);}}
else
{if(this.nextShootTime<=Date.now())
{this.shoot(ig.game.base.pos);}}
...
ig.music.play('GameOver');break;}}},update:function(){if(this.currentLevel==LevelGame_screen){if(this.nextSecond<=Date.now()){waveCountdownInterval();}}
