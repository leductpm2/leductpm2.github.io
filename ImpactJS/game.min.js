...
ig.baked=true;ig.module('game.entities.base').requires('impact.entity').defines(function(){EntityBase=ig.Entity.extend({_wmScalable:true,_wmDrawBox:true,_wmBoxColor:'rgba(0, 255, 0, 0.7)',type:ig.Entity.TYPE.A,checkAgainst:ig.Entity.TYPE.B,size:{x:100,y:80},offset:{x:30,y:30},gravityFactor:0,health:10,animSheet:new ig.AnimationSheet('media/small/island.png',160,140),init:function(x,y,settings){this.parent(x,y,settings);ig.game.base=this;this.addAnim('heal0',1,[5]);this.addAnim('heal1',1,[4]);this.addAnim('heal2',1,[3]);this.addAnim('heal3',1,[2]);this.addAnim('heal4',1,[1]);this.addAnim('heal5',1,[0]);this.currentAnim=this.anims.heal5;},onhealthChange:function(){if(this.health==0){this.currentAnim=this.anims.heal0;ig.game.loadLevelDeferred(LevelGame_over_screen);}
else if(1<=this.health&&this.health<=2){this.currentAnim=this.anims.heal1;}
else if(3<=this.health&&this.health<=4){this.currentAnim=this.anims.heal2;}
else if(5<=this.health&&this.health<=6){this.currentAnim=this.anims.heal3}
else if(7<=this.health&&this.health<=8){this.currentAnim=this.anims.heal4;}
else if(9<=this.health&&this.health<=10){this.currentAnim=this.anims.heal5;}},update:function(){this.parent();this.onhealthChange();},kill:function(){this.parent();ig.game.base_canon.kill();if(highest_score<current_score){highest_score=current_score;}
setTimeout(function(){ig.game.loadLevelDeferred(LevelGame_over_screen);},1000);},});});

// lib/game/entities/base_canon.js
ig.baked=true;ig.module('game.entities.base_canon').requires('impact.entity').defines(function(){EntityBase_canon=ig.Entity.extend({size:{x:72,y:50},flip:false,animSheet:new ig.AnimationSheet('media/small/island_canon.png',72,50),init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('idle',1,[0]);this.addAnim('shoot',0.2,[0,1,2,3,4]);this.currentAnim=this.anims.idle;ig.game.base_canon=this;this.shootPos={x:this.pos.x+this.size.x/2,y:this.pos.y+this.size.y/2}},shoot:function(target){this.currentAnim=this.anims.shoot.rewind();ig.game.spawnEntity(EntityCanonball,this.pos.x+this.size.x/2,this.pos.y+this.size.y/2,{canonPos:this.shootPos,targetPos:target,flip:this.flip});},update:function(){if(ig.input.pressed('shoot')){this.flip=ig.input.mouse.x<this.shootPos.x;this.shoot(ig.input.mouse);}
...
ig.baked=true;ig.module('game.entities.canonball').requires('impact.entity','impact.entity-pool').defines(function(){EntityCanonball=ig.Entity.extend({type:ig.Entity.TYPE.NONE,checkAgainst:ig.Entity.TYPE.B,size:{x:12,y:12},maxVel:{x:1000,y:10000},animSheet:new ig.AnimationSheet('media/small/canon_ball.png',12,12),init:function(x,y,settings){this.parent(x,y,settings);this.vel=calculateCanon(settings.canonPos,settings.targetPos,settings.flip);this.addAnim('shoot',0.2,[0,1,2,3,4,5]);},reset:function(x,y,settings){this.parent(x,y,settings);this.vel=calculateCanon(settings.canonPos,settings.targetPos,settings.flip);},update:function(){this.parent();this.currentAnim.update();if(this.pos.y>1000)
{this.kill();}},check:function(other){other.receiveDamage(1,this);ig.game.spawnEntity(EntityExplosion,other.pos.x+other.size.x/2,other.pos.y+other.size.y/2);this.kill();}});ig.EntityPool.enableFor(EntityCanonball);});
...
ig.baked=true;ig.module('game.entities.bullet_enemy').requires('impact.entity','impact.entity-pool').defines(function(){EntityBullet_enemy=ig.Entity.extend({_wmScalable:true,_wmDrawBox:true,_wmBoxColor:'rgba(255, 0, 0, 0.7)',type:ig.Entity.TYPE.NONE,checkAgainst:ig.Entity.TYPE.A,size:{x:24,y:6},maxVel:{x:1000,y:1000},gravityFactor:0,animSheet:new ig.AnimationSheet('media/small/water_droplet.png',32,24),init:function(x,y,settings){this.parent(x,y,settings);this.flip=settings.flip;this.vel.x=settings.target.x-x;this.vel.y=settings.target.x-y;this.addAnim('shoot',0.2,[0,1,2,3]);this.addAnim('hit',0.2,[4,5,6,7]);this.currentAnim=this.anims.shoot;},reset:function(x,y,settings){this.parent(x,y,settings);this.flip=settings.flip;this.vel.x=settings.target.x-x;this.vel.y=settings.target.x-y;this.gravityFactor=0;this.currentAnim=this.anims.shoot;},update:function(){if((this.currentAnim==this.anims.hit)&&(this.currentAnim.loopCount>0)){this.kill();}
this.currentAnim.flip.x=this.flip;this.currentAnim.update();this.parent();},check:function(other){if(this.currentAnim==this.anims.shoot){this.currentAnim=this.anims.hit.rewind();this.vel.x=0;this.vel.y=0;this.gravityFactor=0;other.receiveDamage(1,this);}}});ig.EntityPool.enableFor(EntityBullet_enemy);});

// lib/game/entities/enemy_ship_2.js
ig.baked=true;ig.module('game.entities.enemy_ship_2').requires('impact.entity').defines(function(){EntityEnemy_ship_2=ig.Entity.extend({_wmScalable:true,_wmDrawBox:true,_wmBoxColor:'rgba(255, 0, 0, 0.7)',type:ig.Entity.TYPE.B,size:{x:132,y:56},offset:{x:0,y:40},gravityFactor:0,ready_to_shoot:false,spawnPos:null,animSheet:new ig.AnimationSheet('media/small/ship_2_sprite.png',132,96),init:function(x,y,settings){this.parent(x,y,settings);this.flip=settings.flip;this.health=settings.maxHealth;this.spawnPos=settings.spawnPos;this.addAnim('idle',1,[0]);this.addAnim('shoot',0.2,[0,1,2,3,4,5]);this.currentAnim=this.anims.idle;this.vel.x=(this.flip?1:-1)*enemy_speed;this.target_posX=this.pos.x+(this.flip?250:-250);},activating_shoot:function(){this.ready_to_shoot=true;this.vel.x=0;var self=this;this.shoot_interval=setInterval(function(){if(!ig.game.base._killed){self.shoot(ig.game.base.pos);}
else{clearInterval(self.shoot_interval);}},enemy_rate_of_fire);},shoot:function(target){this.currentAnim=this.anims.shoot.rewind();ig.game.spawnEntity(EntityBullet_enemy,this.pos.x,this.pos.y,{target:target,flip:this.flip});},update:function(){if(this.currentAnim==this.anims.shoot&&this.currentAnim.loopCount>0){this.currentAnim=this.anims.idle;}
