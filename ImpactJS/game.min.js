...
ig.baked=true;ig.module('game.entities.spawn_point').requires('impact.entity','game.manager').defines(function(){EntitySpawn_point=ig.Entity.extend({_wmScalable:true,_wmDrawBox:true,_wmBoxColor:'rgba(255, 0, 0, 0.7)',flip:false,size:{x:250,y:10},spawned:false,init:function(x,y,settings){this.parent(x,y,settings);this.flip=settings.flip;enemySpawnPoints.push(this);},spawEnemy:function(){if(!this.spawned){if(enemyLeftInWave>0){let type=1+Math.floor(Math.random()*total_enemy_type);ig.game.spawnEntity(ig.global['EntityEnemy_ship_'+type],this.flip?this.pos.x:this.pos.x+this.size.x,this.pos.y,{maxHealth:maxHealthEnemy,flip:this.flip,spawnPos:this,distance:this.size.x});enemyLeftInWave-=1;this.spawned=true;}
else
